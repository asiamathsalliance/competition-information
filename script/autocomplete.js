    ...
    resultBox.querySelectorAll("li").forEach(li => {
      li.addEventListener("click", () => {
        ...
        if (sectionId) {
          ...
          if (section.classList.contains("about-left")) {
            const innerWrapper = section.querySelector(".about-wrapper");
            if (innerWrapper) {
              // 1️⃣ Remove the visible class to reset
              innerWrapper.classList.remove("visible");

              // 2️⃣ Small delay to allow browser to register the removal
              setTimeout(() => {
                innerWrapper.classList.add("visible"); // 3️⃣ Add class to trigger animation
              }, 50); // 50ms is usually enough
            }
          } else {
            // For left/right sections
